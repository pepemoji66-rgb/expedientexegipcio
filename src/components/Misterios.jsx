...
const MISTERIOS_DATA = [
  ...
  {
    id: "acustica-camara",
    titulo: "Acuática y Resonancia a 432 Hz",
    icono: "🔊",
    ...
    <div className="misterios-seccion">
      ...
      <div className="misterios-grid">
        {MISTERIOS_DATA.map((m) => {
          const estaActivo = activoId === m.id;
          return (
            <div key={m.id} className={`misterio-card ${estaActivo ? "activo" : ""}`}>
              ...
              {estaActivo && (
                <div className="card-contenido-detallado">
                  <p>{m.textoCompleto}</p>
                </div>
              )}
              ...
              <button 
                ...
                onClick={() => setActivoId(estaActivo ? null : m.id)}
              >
                {estaActivo ? "OCULTAR ENIGMA 𓂀" : "DESCIFRAR ENIGMA 𓂀"}
              </button>
            </div>
          );
        })}
